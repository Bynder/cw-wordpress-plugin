...
<div id="gc-related-data" data-id="<?php $this->output( 'post_id' ); ?>" data-item="<?php $this->output( 'item_id' ); ?>" data-mapping="<?php $this->output( 'mapping_id' ); ?>" class="no-js gathercontent-admin">
	<?php if ( $this->get( 'mapping_id' ) ) : ?>
		<p><span class="spinner is-active"></span>  <?php esc_html_e( 'Loading...', 'content-workflow-by-bynder' ); ?></p>
	<?php else : ?>
		<p><?php printf( esc_html__( 'This %s does not have an associated item or Template Mapping.', 'content-workflow-by-bynder' ), esc_attr($this->get( 'label' )) ); ?></p>

<# if ( data.statuses.length ) { #>
	<select class="gc-default-mapping-select gc-select2" data-column="gc_status" data-id="{{ data.id }}" name="gc_status">
		<option data-color="" data-description="" <# if ( ! data.status.id ) { #>selected="selected"<# } #> value=""><?php esc_attr_e( 'Unchanged', 'content-workflow-by-bynder' ); ?></option>
...
<# } else { #>
	<span data-id="{{ data.id }}" data-item="{{ data.item }}" data-mapping="{{ data.mapping }}">
		<?php esc_attr_e( 'N\A', 'content-workflow-by-bynder' ); ?>
	</span>
<# } #>
<?php

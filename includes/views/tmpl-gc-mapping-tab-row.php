...
<td <# if (data.typeName === 'component') { #> class="gc-component gc-component-disabled column"<# } #>>
	<# if ( ( data.limit && data.limit_type ) || data.instructions || data.typeName ) { #>
		...
		<# } #>
		<a title="<?php echo esc_html_x( 'Click to show additional details', 'About the Content Workflow object', 'content-workflow-by-bynder' ); ?>" href="#" class="gc-reveal-items <# if(data.component){ #>gc-reveal-items-component<# } #> dashicons-before dashicons-arrow-<# if ( data.expanded ) { #>down<# } else { #>right<# } #>"><strong>{{ data.label }} <small>{{ data.subtitle }}</small></strong></a>
		<ul class="gc-reveal-items-list <# if ( !data.expanded ) { #>hidden<# } #>">
			...
			<li><strong><?php esc_html_e( 'Limit:', 'content-workflow-by-bynder' ); ?></strong> {{ data.limit }} {{ data.limit_type }} </li>
			<# } #>

			<# if ( data.instructions ) { #>
			<li><strong><?php esc_html_e( 'Description:', 'content-workflow-by-bynder' ); ?></strong> {{ data.instructions }}</li>
			<# } #>
		</ul>
	<# } else { #>
...
<td id="<# if (data.typeName !== 'component') { #>{{data.name}}<# } #>" class="<# if (data.typeName === 'component') { #>gc-component gc-component-disabled column<# } #> <# if ( data.is_repeatable ) { #>type-repeater<# } #>">
	<select class="wp-type-select type-select" name="<?php $this->output( 'option_base' ); ?>[mapping][{{ data.name }}][type]">
		<option <# if ( '' === data.field_type ) { #>selected="selected"<# } #> value=""><?php esc_html_e( 'Unused', 'content-workflow-by-bynder' ); ?></option>
					...
					<tr class="{{ field.field_type }} <# if ( field?.metadata?.repeatable?.isRepeatable ) { #>repeater<# } #>">
						<td class="">
							<# if ( field.metadata && field.metadata.repeatable && field.metadata.repeatable.isRepeatable ) { #>
								<span class="dashicons dashicons-controls-repeat" title="Repeatable Field"></span>
							<# } #>
							<a title="<?php echo esc_html_x( 'Click to show additional details', 'About the Content Workflow object', 'content-workflow-by-bynder' ); ?>" href="#" class="gc-reveal-items dashicons-before dashicons-arrow-right">
								<strong>{{ field.label }} <small>{{ field.subtitle }}</small></strong>
							</a>
							<ul class="gc-reveal-items-list gc-reveal-items-hidden hidden">
								<# if(( field.field_type )){ #> <li><strong><?php esc_html_e( 'Type:', 'content-workflow-by-bynder' ); ?></strong> {{ subfield_type_translate[field.field_type] }}</li> <# } #>
								<# if(( field.instructions )){ #> <li><strong><?php esc_html_e( 'Instructions:', 'content-workflow-by-bynder' ); ?></strong> {{ field.instructions }}</li> <# } #>
							</ul>
						</td>

<div class="gc-template-tab-group">
	...
	<?php foreach ( $this->get( 'tabs' ) as $tab_id => $tab ) : ?>
		<a href="#<?php echo esc_attr( $tab['id'] ); ?>" class="nav-tab <?php echo esc_html( $tab['nav_class'] ?? '' ); ?>"><?php echo esc_html($tab['label']); ?></a>
	<?php endforeach; ?>
	</h5>
	...
	<?php foreach ( $this->get( 'tabs' ) as $tab_id => $tab ) : ?>
		<fieldset class="gc-template-tab <?php echo esc_html($tab['tab_class'] ?? ''); ?>" id="<?php echo esc_attr( $tab['id'] ); ?>">

	...
	<tr id="gc-status-<?php echo esc_attr( $status->id ); ?>">
		...
		<td>
			<select class="gc-default-mapping-select" data-column="post_status_mapping" name="<?php $this->output( 'option_base' ); ?>[gc_status][<?php echo esc_attr( $status->id ); ?>][wp]">
				<option <# if ( ! data.gc_status[<?php echo esc_attr( $status->id ); ?>] || ! data.gc_status[<?php echo esc_attr( $status->id ); ?>].wp ) { #>selected="selected"<# } #> value=""><?php _e( 'Use Default Status', 'content-workflow-by-bynder' ); ?></option>
		...
		<td>
			<select class="gc-default-mapping-select gc-select2" data-column="gc_status" name="<?php $this->output( 'option_base' ); ?>[gc_status][<?php echo esc_attr( $status->id ); ?>][after]"">
				<option <# if ( ! data.gc_status[<?php echo esc_attr( $status->id ); ?>] || ! data.gc_status[<?php echo esc_attr( $status->id ); ?>].after ) { #>selected="selected"<# } #> value=""><?php _e( 'Do not change', 'content-workflow-by-bynder' ); ?></option>

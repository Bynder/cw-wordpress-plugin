<div class="misc-pub-section gc-item-name">
	<span class="dashicons dashicons-edit"></span> <?php echo esc_html_x( 'Item:', 'Content Workflow item name', 'gathercontent-importer' ); ?> <# if ( data.item ) { #><a href="<?php $this->output( 'url' ); ?>item/{{ data.item }}" target="_blank" title="<?php esc_attr_e( 'Item ID:', 'gathercontent-importer' ); ?> {{ data.item }}"><# } #>{{ data.itemName }}<# if ( data.item ) { #></a><# } #>
</div>

<div class="misc-pub-section misc-gc-updated">
	<span class="dashicons dashicons-calendar"></span> <?php echo esc_html_x( 'Last Updated:', 'Content Workflow updated date', 'gathercontent-importer' ); ?> <b><?php echo new self( 'underscore-data-updated', $this->args ); ?></b>
</div>

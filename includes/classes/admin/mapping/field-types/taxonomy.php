...
class Taxonomy extends Base implements Type {
	...
	public function underscore_template( View $view ) {
		foreach ( $this->post_types as $type ) : ?>
			<# if ( '<?php echo esc_js( $this->e_type_id() ); ?>' === data.field_type && '<?php echo esc_js( $type->name ); ?>' === data.post_type ) { #>
			<select
				class="wp-type-value-select <?php echo esc_attr( $this->e_type_id() ); ?> wp-taxonomy-<?php echo esc_attr( $type->name ); ?>-type"

...
class Sync_Items extends Plugin_Base {
	...
	protected function check_http_auth() {
		...
		if (
			...
			wp_send_json_error(
				array(
					'message' => __( 'Syncing is disabled until authentication credentials are provided. Redirecting to the settings page.', 'content-workflow-by-bynder' ),
	...
	protected function verify_nonce() {
		...
		if ( ! wp_verify_nonce( $this->_post_val( 'nonce' ), $opt_group . '-options' ) ) {
			wp_send_json_error(
				sprintf(
					__( 'Error %d: Missing security nonce.', 'content-workflow-by-bynder' ),
					__LINE__
				)
			);
		}
	...
	protected function get_fields() {
		...
		if ( empty( $data ) || ! is_string( $data ) ) {
			wp_send_json_error(
				sprintf(
					__( 'Error %d: Missing form data.', 'content-workflow-by-bynder' ),
					__LINE__
				)
			);
		}
		...
		if (
			...
			wp_send_json_error(
				sprintf(
					__( 'Error %d: Missing required form data.', 'content-workflow-by-bynder' ),
					__LINE__
				)
			);
		}

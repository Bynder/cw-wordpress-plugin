...
class Push extends Base {
	...
	protected function do_item( $id ) {
		...
		if ( empty( $config_update ) ) {
			...
			throw new Exception(
				sprintf( esc_attr__( 'No update data found for that post ID: %d', 'content-workflow-by-bynder' ), esc_attr($this->post->ID) ),
				__LINE__,
				array(
					'post_id'    => esc_attr($this->post->ID),
					'mapping_id' => esc_attr($this->mapping->ID),
					'item_id'    => esc_attr($this->item->id) ?? 0,
				)
			);
		}

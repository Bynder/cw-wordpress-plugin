...
class Mapping_Post extends Base {
	...
	protected static function get_post( $post ) {
		...
		if ( ! $post ) {
			throw new Mapping_Post_Exception( __CLASS__ . ' expects a WP_Post object or post ID.', __LINE__, esc_attr($post) );
		}

		if ( Template_Mappings::SLUG !== $post->post_type ) {
			throw new Mapping_Post_Exception( __CLASS__ . ' expects a ' . esc_attr(Template_Mappings::SLUG) . ' object.', __LINE__, esc_attr($post->post_title) );
		}
	...
	public function __get( $property ) {
		...
		switch ( $property ) {
			...
			default:
				...
				throw new Mapping_Post_Exception( 'Invalid ' . __CLASS__ . ' property: ' . esc_attr($property) );
		}
	}
